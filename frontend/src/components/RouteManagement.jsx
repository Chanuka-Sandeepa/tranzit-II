...
const RouteManagement = ({ routes, fetchRoutes }) => {
  ...
  const handleSubmitRoute = async (e) => {
    e.preventDefault();
    try {
      const url = currentRoute ? `http://localhost:8000/api/route/${currentRoute._id}` : 'http://localhost:8000/api/route';
  ...
  const handleDeleteRoute = async (id) => {
    if (window.confirm('Are you sure you want to delete this route?')) {
      try {
        const response = await fetch(`http://localhost:8000/api/route/${id}`, {
          method: 'DELETE',
        });
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
